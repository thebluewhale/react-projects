...
const mailContents = '이 링크를 눌러 가입을 완료해주세요 : ';
		...
		if (error) {
			...
			newAccount.save((error) => {
				if (error) {
					...
					let confirmLink = `localhost:8080/accountconfirm/${username}/`;
					let hashvalue = crypto.createHash('sha256').update(username, 'utf8').digest('hex');;
					confirmLink = confirmLink + hashvalue;
					let mailOption = {
						from: mailFrom,
						to: 'beat410@naver.com',
						subject: mailTitle,
						text: mailContents + confirmLink
					};
