...
export function getAllMarkersRequest(username) {
	return (dispatch) => {
		...
		let fullURL = DEFAULT_URL + `/marker/getallmarkers/${username}`;
...
export function getMarkersByKeywordRequest(username, keyword) {
	return (dispatch) => {
		dispatch(getMarkersByKeyword());

		let paramQuery = `?username=${username}&keyword=${keyword}`
		let fullURL = DEFAULT_URL + `/marker/getmarkers/bykeyword${paramQuery}`;
		return axios.get(fullURL).then((response) => {
			dispatch(getMarkersByKeywordSuccess(response.data));
		}).catch((error) => {
