...
export default class coupangCrawler {
	constructor() {
		this.company = 'coupang';
		this.name = '쿠팡';
		this.links = [];
		this.url = 'https://boards.greenhouse.io/coupang1?gh_src=kqxsb2#.WU0q9B8xDCI';
		//https://boards.greenhouse.io/coupang2?gh_src=wj6iyx#.WU0q6h8xDCI
	...
	getPageLinks() {
		return new Promise((resolve) => {
			this.links = [
				'https://boards.greenhouse.io/coupang1?gh_src=kqxsb2#.WU0q9B8xDCI',
				'https://boards.greenhouse.io/coupang2?gh_src=kqxsb2#.WU0q9B8xDCI'
			];
			return resolve();
		});
	}
	...
	refineData(rawData) {
		...
		for(let e in rawData.link) {
			let defaultURL = 'https://boards.greenhouse.io';
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('#main .opening a').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "let link = $('#main .opening a').map(function() {";
		...
		driver.wait(until.titleContains('Coupang'), 5000).catch((err) => {
			return;
		});
		let that = this;
