...
export default class ebayKoreaCrawler {
	constructor() {
		this.company = 'ebayKorea';
		this.name = '이베이코리아';
		this.links = [];
		this.url = 'https://recruit.ebaykorea.com/jobinfo';
	...
	refineData(rawData) {
		...
		for(let e in rawData.link) {
			let defaultURL = 'https://recruit.ebaykorea.com/jobinfo/';
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('#content tbody tr td.title a').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "let link = $('#content tbody tr td.title a').map(function() {";
			dataCommand += "return $(this).attr('href');";
			dataCommand += "}).get();";
			dataCommand += "let period = $('#content tbody tr td:nth-child(5)').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "";
			dataCommand += "return {";
		...
		driver.wait(until.titleContains('이베이코리아'), 5000).catch((err) => {
			return;
		});
		let that = this;
