...
export default class tmonCrawler {
	constructor() {
		this.company = 'tmon';
		this.name = '티몬';
		this.links = [];
		this.url = 'https://recruit.ticketmonster.co.kr/recruit/list/000#page=1';
	...
	getPageLinks() {
		return new Promise((resolve) => {
			...
			driver.wait(until.titleContains('티몬'), 5000).catch((err) => {
				return;
			});

			let that = this;
			let getDataCommand = "return $('#pagerDiv span.num_page a:last-child').text();";
			driver.executeScript(getDataCommand).then(function(lastPage) {
				that.links = [];
				for(let i = 1; i <= lastPage; i++) {
					that.links.push('https://recruit.ticketmonster.co.kr/recruit/list/000#page=' + i);
				}
			}).catch((err) => {
				return;
			});
	...
	refineData(rawData) {
		...
		for(let e in rawData.link) {
			let defaultURL = 'https://recruit.ticketmonster.co.kr';
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('#content tbody tr td.align_left a').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "let link = $('#content tbody tr td.align_left a').map(function() {";
			dataCommand += "return $(this).attr('href');";
			dataCommand += "}).get();";
			dataCommand += "let period = $('#content tbody tr td span.t_date').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "";
			dataCommand += "return {";
