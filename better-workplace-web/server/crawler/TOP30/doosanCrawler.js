...
export default class doosanCrawler {
	constructor() {
		this.company = 'doosan';
		this.name = '두산';
		this.links = [];
		this.url = 'https://career.doosan.com/dsp/sa/RecList.jsp';
	...
	refineData(rawData) {
        ...
        for(let e in rawData.title) {
            titleData.push(`[${rawData.affiliate[e]}] ${rawData.title[e]}`);
        }
	...
	executeCrawling(url) {
		...
		let dataCommand = "let affiliate = $('.tblList tbody tr td:nth-child(2)').map(function() {";
            dataCommand += "return $(this).text();";
            dataCommand += "}).get();";
            dataCommand += "let title = $('.tblList tbody tr td:nth-child(3)').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
            dataCommand += "let period = $('.tblList tbody tr td:nth-child(4)').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
			dataCommand += "";
			dataCommand += "return {";
		...
		driver.wait(until.titleContains('DOOSAN'), 5000).catch((err) => {
			return;
		});
		let that = this;
