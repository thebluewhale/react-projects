...
export default class lgCrawler {
	constructor() {
		this.company = 'lg';
		this.name = 'LG';
		let links = [];
		this.url = 'http://apply.lg.com/app/job/RetrieveJobNotices.rpi';
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('#LtableJobNoticesList tbody tr').map(function() {";
			dataCommand += "let compName = $(this).find('td.companyName').text();";
			dataCommand += "let jobDesc = $(this).find('td.LtextLeft a').text();";
			dataCommand += "return compName + ' : ' + jobDesc;";
			dataCommand += "}).get();";
			dataCommand += "let link = $('#LtableJobNoticesList tbody tr td.companyName').map(function() {";
