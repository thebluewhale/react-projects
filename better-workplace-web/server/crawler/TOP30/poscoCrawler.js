...
export default class poscoCrawler {
	constructor() {
		this.company = 'posco';
		this.name = '포스코';
		this.links = [];
		this.url = 'http://gorecruit.posco.net/H22/H22010/h220101010.do?ServiceName=h220101010-service&find=1';
	...
	refineData(rawData) {
		...
		for(let e in rawData.link) {
            let defaultURL = 'http://gorecruit.posco.net/H22/H22010/h220101020.do?ServiceName=h220101020-service&find=1&Recu010IdP=';
            let part = rawData.link[e];
            if(part && part.split('goDetailInfo')[1] && part.split('goDetailInfo')[1].split(')')[0]) {
                part = rawData.link[e].split('goDetailInfo(')[1].split(')')[0];
            } else {
                part = this.url;
            }
			linkData.push(defaultURL + part);
		}

		for(let e in rawData.period) {
            let part = rawData.period[e];
            if(part.split && part.split('~')[1].split) {
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('table a.cutSt').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
            dataCommand += "let period = $('table .tblcw td:nth-child(3)').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
            dataCommand += "let link = $('table a.cutSt').map(function() {";
		...
		driver.wait(until.titleContains('POSCO'), 5000).catch((err) => {
			return;
		});
		let that = this;
