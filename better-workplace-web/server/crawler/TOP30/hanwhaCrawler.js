...
export default class hanwhaCrawler {
	constructor() {
		this.company = 'hanwha';
		this.name = '한화';
		this.links = [];
		this.url = 'http://www.hanwhain.com/web/apply/notification/list_iframe.do?schTp=&schNm=';
	...
	//*/
	refineData(rawData) {
		let titleData = [], linkData = [], expireData = [];
        ...
        for(let e in rawData.title) {
            if(rawData.title[e].replace) {
                titleData.push(rawData.title[e].replace('\n', ' - '));
            }
        }

		for(let e in rawData.link) {
            let defaultURL = 'http://www.hanwhain.com/web/apply/notification/view.do?rtSeq=';
	...
	executeCrawling(url) {
		...
		let dataCommand = "let title = $('.block p strong.job').map(function() {";
			dataCommand += "return $(this)[0].innerText;";
			dataCommand += "}).get();";
            dataCommand += "let period = $('.block-list ul li em.period').map(function() {";
			dataCommand += "return $(this).text();";
			dataCommand += "}).get();";
            dataCommand += "let link = $('.block-list ul li input').map(function() {";
            dataCommand += "return $(this)[0].value;";
            dataCommand += "}).get();"
			dataCommand += "";
			dataCommand += "return {";
		...
		driver.wait(until.titleContains('한화'), 5000).catch((err) => {
			return;
		});
		let that = this;
