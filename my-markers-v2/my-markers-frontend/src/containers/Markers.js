...
class Markers extends React.Component {
		...
		if (this.state.selectedMarker && this.state.selectedMarker._id) {
			this.props.deleteMarkerRequest(this.state.selectedMarker._id).then(() => {
				...
				if (this.props.delete.status === 'SUCCESS') {
					...
					window.M.toast({html: '삭제되었습니다.', classes: 'styled-font', displayLength: 2000});
				} else {
					window.M.toast({html: '삭제에 실패했습니다.', classes: 'styled-font', displayLength: 2000});
				}
			});
		}
	}
		...
		if (this.state.selectedMarker && this.state.selectedMarker._id) {
			this.props.updateMarkerRequest(this.state.selectedMarker._id, data).then(() => {
				...
				if (this.props.update.status === 'SUCCESS') {
					...
					window.M.toast({html: '수정에 실패했습니다.', classes: 'styled-font', displayLength: 2000});
				}
			});
		}
	}
	...
	handleAddMarker() {
		...
		this.props.addNewMarkerRequest(data).then(() => {
			...
			if (this.props.addNew.status === 'SUCCESS') {
				window.M.toast({html: '저장되었습니다.', classes: 'styled-font', displayLength: 2000});
			} else {
				window.M.toast({html: '저장에 실패했습니다.', classes: 'styled-font', displayLength: 2000});
			}
		});
	...
	setPageStatusToAdd() {
		...
		this.infoWindow.close();
		window.M.toast({html: '새로운 장소를 등록합니다.', classes: 'styled-font', displayLength: 2000});
	}
	...
	setPageStatusToSearch() {
		...
		this.removeAllMarkers();
		this.infoWindow.close();
		window.M.toast({html: '등록한 장소를 검색할 수 있습니다.', classes: 'styled-font', displayLength: 2000});
	...
	displayUserMarker(mode) {
		...
		if (data.length === 0) {
			window.M.toast({html: '검색된 데이터가 없습니다.', classes: 'styled-font', displayLength: 2000});
			return;
		}
				...
				this.props.getMarkersByKeywordRequest(keyword).then(() => {
					if (this.props.getByKeyword.status === 'SUCCESS') {
						if (this.props.getByKeyword.markers.length) {
							window.M.toast({html: '검색 결과입니다.', classes: 'styled-font', displayLength: 2000});
							this.displayUserMarker('FILTERED');
						} else {
							window.M.toast({html: '검색 결과가 없습니다.', classes: 'styled-font', displayLength: 2000});
						}
					} else {
						window.M.toast({html: '검색에 실패했습니다.', classes: 'styled-font', displayLength: 2000});
					}
				});
			}
		}
					...
					<div className='row'>
						...
						<div className='col s12 m12 l12'>
							<div className='input-field styled-font black-text'>
								...
								<label className='styled-font black-text'>카테고리 선택</label>
							</div>
						</div>
						<form className='col s12 m12 l12'>
							<div className='row'>
								<div className='input-field col s12 m12 l12'>
									...
									<label className='styled-font black-text' htmlFor='modal-input'>간단한 메모를 입력하세요</label>
								</div>
							</div>
						</form>
					</div>
